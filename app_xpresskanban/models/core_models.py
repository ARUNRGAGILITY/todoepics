...
class CORE_HSDB(MPTTModel):
    ...
    # mapping
    vsm = models.ForeignKey(VSM, on_delete=models.CASCADE, related_name='vsm_vs1', null=True, blank=True)
    ...
    label =  models.CharField(max_length=256,null=True, blank=True, default='')
    blocked =  models.CharField(max_length=256,null=True, blank=True, default='')
    block_count = models.PositiveIntegerField(default=0)
    member =  models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='XPRESS_CORE_HSDB_MEMBER')
    # trying to capture, like bug, cr, feature etc., quickly 
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
